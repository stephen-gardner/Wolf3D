...
/*   Updated: 2018/02/14 00:05:17 by sgardner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_vert(t_map *map, t_raycast *rc)
{
	...
	y_start = rc->y_start;
	y_end = rc->y_end;
	while (y < y_start - 1 && y < y_end)
		*(cvs->data + (cvs->x_max * y++) + x) = (0xE5E5FF + (map->mod % 5));
	if (y < y_start)
		*(cvs->data + (cvs->x_max * y++) + x) = 0;
	while (y < y_end)
	{
		*(cvs->data + (cvs->x_max * y) + x) = get_color(rc, y, map->mod);
		++y;
	}
	if (y < cvs->y_max - 1)
		*(cvs->data + (cvs->x_max * y++) + x) = 0;
	while (y < cvs->y_max - 1)
		*(cvs->data + (cvs->x_max * y++) + x) = (0x006400 + (map->mod % 80));
}
...
static void	calc_wallpoints(t_map *map, t_raycast *rc, int step_x, int step_y)
{
	...
	rc->y_start = ((map->cvs->y_max - line_height) / 2) + pos->mod_z;
	if (rc->y_start < 0)
		rc->y_start = 0;
	rc->y_end = ((map->cvs->y_max + line_height) / 2) + pos->mod_z;
	if (rc->y_end >= map->cvs->y_max)
		rc->y_end = map->cvs->y_max - 1;
}
...
static int	find_wall(t_map *map, t_raycast *rc, int step_x, int step_y)
{
	...
	side = 0;
	while (!map->arr[rc->map_y][rc->map_x])
	{
